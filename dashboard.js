...
function applyFilters(activities = state.activities) {
  const search = key(state.filters.search);
  return activities.filter((item) => {
    const status = item.done ? "Completed" : "Pending";
    const haystack = key([item.module, item.country, item.school, item.responsible, item.title, item.notes].join(" "));
    return (
      (state.filters.country === "All" || item.country === state.filters.country) &&
      (state.filters.school === "All" || item.school === state.filters.school) &&
...
function calculateKPIs(activities) {
  const countries = table("countries").map((row) => normalizeEntity(row, "Country"));
  const schools = state.schools;
  const teachers = table("teachers");
  const students = table("students");
  const priests = table("priests");
  ...
  const pending = activities.filter((item) => !item.done).length;
  const overdue = activities.filter((item) => item.dueDate && item.dueDate < new Date() && !item.done).length;
  const total = completed + pending;
  return [
    ["Total Countries", countries.length, "Registered countries", "CO"],
    ["Active Countries", countries.filter(isActive).length, "Countries currently active", "AC"],
    ["Total Schools", schools.length, "Registered schools", "SC"],
    ["Active Schools", schools.filter(isActive).length, "Schools currently active", "AS"],
    ["Total Students", students.length, "Students in all records", "ST"],
    ["Total Teachers", teachers.length, "Teachers in all records", "TE"],
    ["Total Priests", priests.length, "Priests in service records", "PR"],
    ["Total Residents", residents.length, "Residents in service records", "RE"],
    ["Total Principals", principals.length, "Principals in records", "PI"],
    ["Total Team Members", team.length, "Team members in records", "TM"],
    ["Completed Tasks", completed, "Done activity records", "DN", "good"],
    ["Pending Tasks", pending, "Open activity records", "PN", pending ? "warning" : "good"],
    ["Overdue Tasks", overdue, "Past due and not done", "OD", overdue ? "critical" : "good"],
    ["Overall Completion Rate", total ? `${Math.round((completed / total) * 100)}%` : "No records", "Completed tasks / all tasks", "CR"],
    ["Published News", news.filter((item) => item.published).length, "Visible news records", "NW"],
    ["Active Projects", projects.filter(isActive).length, "Open project records", "PJ"],
    ["Buildings In Progress", buildings.filter((row) => key(get(row, ["Status", "Phase"])).includes("progress")).length, "Building records in progress", "BD"],
  ];
}
...
function calculateSchoolHealth(activities) {
  const students = table("students").map((row) => normalizeEntity(row, "Student"));
  const teachers = table("teachers").map((row) => normalizeEntity(row, "Teacher"));
  const buildings = table("buildings");
  const news = getNews();
  return state.schools
    .map((school) => {
      const schoolActivities = activities.filter((item) => key(item.school) === key(school.name));
      const schoolStudents = students.filter((item) => key(item.school) === key(school.name));
      const schoolTeachers = teachers.filter((item) => key(item.school) === key(school.name));
      ...
      const hasBuilding = buildings.some((row) => key(get(row, ["School", "School Name"])) === key(school.name));
      const hasNews = news.some((item) => key(item.school) === key(school.name) || (item.date && lastActivity && Math.abs(item.date - lastActivity) < 86400000 * 31));
      const score =
...
function calculateCountrySummary(schoolHealth, activities) {
  const students = table("students").map((row) => normalizeEntity(row, "Student"));
  const teachers = table("teachers").map((row) => normalizeEntity(row, "Teacher"));
  const countries = unique([...state.countries.map((item) => item.name), ...schoolHealth.map((item) => item.country)]);
  return countries.map((country) => {
    const schools = schoolHealth.filter((item) => item.country === country);
    const countryActivities = activities.filter((item) => item.country === country);
    ...
    return {
      ...
      students: students.filter((item) => item.country === country).length,
      teachers: teachers.filter((item) => item.country === country).length,
...
function getNews() {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      school: get(row, ["School", "School Name"]),
...
function detectDataQualityIssues() {
  const issues = [];
  const push = (priority, type, entity, recommendation) => issues.push({ priority, type, entity, recommendation });
  state.schools.forEach((school) => {
    if (!school.country) push("High", "School missing country", school.name, "Assign the school country.");
    if (!school.type) push("Medium", "School missing type", school.name, "Set Formal, Non-Formal, or Informal.");
  });
  table("students").map((row) => normalizeEntity(row, "Student")).forEach((student) => {
    if (!student.name) push("High", "Student missing name", student.school, "Add the student name.");
    if (!student.school) push("High", "Student without school", student.name, "Assign the student to a school.");
    if (!student.className) push("Medium", "Student missing grade/class", student.name, "Add grade or class.");
    if (!student.phone) push("Low", "Student missing contact info", student.name, "Add guardian or contact data.");
  });
  table("teachers").map((row) => normalizeEntity(row, "Teacher")).forEach((teacher) => {
    if (!teacher.school) push("High", "Teacher without school", teacher.name, "Assign the teacher to a school.");
    if (!teacher.phone) push("Low", "Teacher missing phone", teacher.name, "Add phone/contact info.");
  });
  state.activities.forEach((activity) => {
    if (!activity.date) push("Medium", "Activity without date", activity.title, "Add a valid activity date.");
    if (!activity.doneRaw) push("Medium", "Activity without Done status", activity.title, "Set Done/Pending status.");
    if (activity.school && !state.schools.some((school) => key(school.name) === key(activity.school))) {
      push("High", "Log connected to unknown school", activity.school, "Match the log school name with the Schools table.");
    }
  });
  getNews().forEach((news) => {
    if (!news.image) push("Low", "News without image", news.description, "Add a public image URL.");
  });
  return issues;
}

function detectAlerts(schoolHealth, activities) {
  const alerts = [];
  ...
  getNews().filter((item) => !item.published).forEach((item) => add("Medium", "News hidden", item.description, item, "Review whether this news should be published."));
...
function renderCharts(activities, schoolHealth) {
  ...
  byId("chartGrid").innerHTML = [
    chartCard("Done vs Pending", donePending, "Completion status across all activity modules."),
    chartCard("Activities by Module", countsBy(activities, "module"), "Unified logs by service module."),
    chartCard("Activities by Country", countsBy(activities, "country"), "Activity volume by country."),
    chartCard("Schools Health Distribution", health, "School status based on the calculated health score."),
    chartCard("Activities by Month", byMonth, "Recent monthly service movement."),
    chartCard("Pending by Responsible Person", countsBy(activities.filter((item) => !item.done), "responsible"), "Open tasks grouped by responsible person."),
  ].join("");
}
...
function dataTable(id, rows, columns) {
  if (!rows.length) return emptyState("No records yet", "This section will update when records are published.");
...
function renderTables(schoolHealth, countrySummary, activities, issues) {
  ...
  byId("qualityDashboard").innerHTML = `<div class="mini-grid">
    <div class="mini-card"><span>${issues.length}</span><strong>Total data issues</strong></div>
...
function renderPeople() {
  const cards = [
    ...
    ["Teachers", table("teachers").length],
    ["Priests", table("priests").length],
    ["Principals", table("principals").length],
    ["Residents", table("residents").length],
    ["Team Members", table("team").length],
  ];
...
function renderFormation(activities) {
  ...
  byId("formationDashboard").innerHTML = formation.length ? `<div class="mini-grid">
    <div class="mini-card"><span>${formation.length}</span><strong>Formation logs total</strong></div>
  ...
  </div>${chartCard("Formation by School", countsBy(formation, "school"), "Formation coverage by school.")}` : emptyState("No recent formation activity recorded", "Formation progress will appear when logs are published.");
}

function renderProjects() {
  const projects = table("projects");
  const buildings = table("buildings");
  byId("projectsDashboard").innerHTML = projects.length || buildings.length ? `<div class="mini-grid">
    <div class="mini-card"><span>${projects.length}</span><strong>Total Projects</strong></div>
    <div class="mini-card"><span>${projects.filter(isActive).length}</span><strong>Active Projects</strong></div>
    <div class="mini-card"><span>${buildings.length}</span><strong>Building Records</strong></div>
  </div>` : emptyState("No active building or project records yet.", "Projects and building status cards will appear when records are published.");
}

function renderNews() {
  const news = getNews().sort((a, b) => (b.date || 0) - (a.date || 0));
  byId("newsDashboard").innerHTML = news.length ? `<div class="mini-grid">
    <div class="mini-card"><span>${news.length}</span><strong>Total News</strong></div>
  ...
  `).join("")}</div>` : emptyState("No news records yet", "Latest published service updates will appear here.");
}

function populateFilters(activities) {
  const options = {
    filterCountry: ["All", ...unique(activities.map((item) => item.country).concat(state.schools.map((item) => item.country)))],
    filterSchool: ["All", ...unique(activities.map((item) => item.school).concat(state.schools.map((item) => item.name)))],
    filterModule: ["All", ...unique(activities.map((item) => item.module))],
    filterStatus: ["All", "Completed", "Pending"],
    filterPriority: ["All", ...unique(activities.map((item) => item.priority))],
  };
...
function render() {
  ...
  const schoolHealth = calculateSchoolHealth(filteredActivities);
  const countrySummary = calculateCountrySummary(schoolHealth, filteredActivities);
  const issues = detectDataQualityIssues();
  const alerts = detectAlerts(schoolHealth, filteredActivities);
  renderKpis(calculateKPIs(filteredActivities));
  renderAlerts(alerts);
  renderCharts(filteredActivities, schoolHealth);
  renderTables(schoolHealth, countrySummary, filteredActivities, issues);
  renderPeople();
  renderFormation(filteredActivities);
  renderProjects();
  renderNews();
  bindTableTools();
}
...
async function loadDashboard() {
  ...
  try {
    ...
    state.schools = table("schools").map((row) => ({
      raw: row,
    ...
    state.activities = normalizeActivities();
